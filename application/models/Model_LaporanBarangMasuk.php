<?php
class Model_LaporanBarangMasuk extends CI_Model
{
	var $table = 'purchase';
	...
	var $order = array('' => 'asc'); // default order  	private $db_sts;
	...
	private function _get_datatables_query()
	{
		...
		$this->db->join("purchase","prc_id = prcd_prc_id", "left");
		$this->db->join("pengajuan_fix","pgjf_id = prc_pgjf_id", "left");
		$this->db->join("pengajuan","pgj_id = pgjb_id", "left");
		$this->db->join("request","rq_id = pgj_id_rq", "left");
		$this->db->join("project","id_project = id_prj", "left");
		$this->db->join("barang","brg_id = prcd_brg_id", "left");
		$this->db->join("status_pengajuan","sp_id_pgjf = prc_pgjf_id", "left");
		$this->db->where('status_pgjf', 6);
		$this->db->or_where('status_pgjf', 7);
